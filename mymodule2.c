...
void vinegere_cipher (int arg, struct idNode currIdNode, char * text) {
  ...
  if (arg == 1) {
    ...
    newKey[i] = '\0';
    strcpy(currIdNode.key, newKey); // Update the key
    strcpy(currIdNode.key, newKey); // Update the message
    ...
    encryptedMsg[i] = '\0';
    strcpy(currIdNode.data, encryptedMsg); // Update the message
  } else {
    // Decryption
    for (i = 0; i < textLen; i++) {
    ...
    decryptedMsg[i] = '\0';
    strcpy(currIdNode.data, decryptedMsg); // Update the message
  }
  return;
}
...
static ssize_t decrypt_read (struct file * filep, char * buffer, size_t len, loff_t * offset) {
  ...
  // copy_to_user returns 0 on success
  // For decryption to be read back
  ret = copy_to_user(buffer, newDecNode.data, strlen(newDecNode.data));
...
static ssize_t encrypt_write (struct file * filep, const char * buffer, size_t len, loff_t * offset) {
  ...
  sprintf(newEncNode.data, "%s", buffer); // Appending received string with its length
  //vinegere_cipher(1, index, newEncNode.data);
  vinegere_cipher(1, newEncNode, newEncNode.data);
...
*/
    ...
    case IOCTL_CHANGE_KEY:
      ...
      //sprintf(key, "%s", virtualDevice.data); // Input in the form KEY INDEX (space in between)
      /* indexchar = strtok(virtualDevice.data, " "); */
      /* sprintf(index, "%d", indexchar); */
	...
	if (strncmp(newEncNode.index, index, index_size) == 0){
      ...
      printk(KERN_INFO "ioctl: key changed successfully\n");
      return 0;
